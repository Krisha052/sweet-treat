...
| CaramelLatte dish icon aesthetic mismatch | `CaramelLatte.png` (the dish icon) still doesn't match the rest of the game's art style. `caramel.png` (the ingredient icon) was fixed with developer-provided art — this is the remaining half of the original flag. No replacement art provided yet. |
